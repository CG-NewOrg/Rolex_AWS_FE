        ...
        getUserinfo: function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    var fullName = [data.firstname, data.lastname].filter(Boolean).join(" ") || data.displayName || "";
                    ...
                    if (Array.isArray(allModels)) {
                        ...
                        orchestrationModels = allModels
                            .filter(function (model) {
                                ...
                                const isAmazonModel = modelName.includes("amazon") || modelName.includes("nova") || modelName.includes("titan");

                                return !isEmbeddingModel && hasNonDeprecatedVersion && isOrchestrationAllowed && !isHiddenModel && !isAmazonModel;
                            })
                            .map(function (model) {
                                const nonDeprecatedVersions = (Array.isArray(model.versions) ? model.versions : []).filter(function (v) {
                                    return v && (v.deprecated === false || v.deprecated === "false");
